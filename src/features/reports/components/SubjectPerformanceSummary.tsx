...
import type { SubjectSummaryItem } from '../types/reports.types'
...
export const SubjectPerformanceSummary: React.FC<SubjectPerformanceSummaryProps> = ({
  subjectSummary = [],
  isLoading = false,
}) => {
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl p-5 md:p-6 shadow-xs space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 font-sans mt-0.5">
            Subject-wise score averages, highest marks, and pass rates.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="h-48 flex flex-col items-center justify-center text-slate-400 space-y-2">
          ...
          <span className="text-xs font-medium">Loading subject summary...</span>
        </div>
        ...
        <div className="h-48 flex items-center justify-center text-slate-400 text-xs font-medium border border-dashed border-slate-200 rounded-xl">
          No subject performance metrics found.
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
          {subjectSummary.map((s) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold font-urbanist text-slate-900">{s.subject}</span>
              ...
              <div className="flex items-center justify-between text-xs text-slate-500 font-sans">
                ...
                <span className="text-emerald-700 font-semibold">{s.pass_rate}% Pass</span>
                <span className="text-amber-700 font-semibold">Max: {s.highest_score}</span>
              </div>

              <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                <div
                  className="bg-brand-primary h-full rounded-full transition-all"
                  style={{ width: `${Math.min(100, Math.max(0, s.avg_score))}%` }}
                />
              </div>
            </div>
          ))}
